...
export default function VerificationPaymentPage() {
  ...
  const crd = (sel) => ({ background:sel?'rgba(184,150,46,0.08)':'rgba(255,255,255,0.03)', border:'1px solid '+(sel?'#b8962e':'rgba(0,0,0,0.1)'), borderRadius:12, padding:20, marginBottom:12, cursor:'pointer' });
  const NAV = <div style={{ background:'#ece8df', borderBottom:'1px solid rgba(184,150,46,0.15)', padding:'12px 24px', display:'flex', alignItems:'center', gap:16 }}><a href="/" style={{ display:'flex', alignItems:'center', gap:8, textDecoration:'none' }}><span style={{ color:'#b8962e', fontSize:18 }}>◈</span><span style={{ color:'#f0ebe0', fontSize:14, fontWeight:600 }}>SAREGO</span></a><span style={{ color:'rgba(232,224,208,0.2)' }}>|</span><a href="/dashboard" style={{ color:'rgba(26,26,26,0.55)', fontSize:13, textDecoration:'none' }}>Dashboard</a><a href="/account" style={{ color:'rgba(26,26,26,0.55)', fontSize:13, textDecoration:'none', marginLeft:'auto' }}>My Account</a></div>;
  if (done) return <div style={pg}>{NAV}<div style={{ ...inn, textAlign:'center', paddingTop:60 }}><div style={{ fontSize:48, marginBottom:16 }}>✅</div><h2 style={{ fontSize:24, fontWeight:700, marginBottom:12 }}>Verification Order Submitted</h2><p style={{ color:'rgba(26,26,26,0.65)', marginBottom:32 }}>Our team will review within {tier.turnaround}.</p><Link to="/account" style={{ background:'#b8962e', color:'#f5f0e8', padding:'12px 32px', borderRadius:8, fontWeight:700, textDecoration:'none' }}>View Account</Link></div></div>;
  return (
    <div style={pg}>{NAV}<div style={inn}>
      <h1 style={{ fontSize:28, fontWeight:700, margin:'32px 0 8px' }}>Verification</h1>
      <p style={{ fontSize:14, color:'rgba(26,26,26,0.55)', margin:'0 0 32px' }}>Build institutional trust on SAREGO</p>
      {TYPES.map(t => (<div key={t.id} style={crd(selected===t.id)} onClick={()=>setSelected(t.id)}><div style={{ display:'flex', justifyContent:'space-between' }}><div style={{ fontSize:16, fontWeight:600 }}>{t.label}</div><div style={{ fontSize:28, fontWeight:800, color:'#b8962e' }}>${t.price}</div></div><div style={{ fontSize:13, color:'rgba(26,26,26,0.65)', marginTop:6 }}>{t.desc}</div><div style={{ fontSize:12, color:'#b8962e', marginTop:8 }}>Turnaround: {t.turnaround}</div></div>))}
      <div style={sec}>
        ...
        <div style={{ fontSize:13, color:'rgba(26,26,26,0.55)', lineHeight:1.6, padding:'12px 0', borderTop:'1px solid rgba(255,255,255,0.06)' }}>Send ${tier&&tier.price} USD to <strong style={{ color:'#f0ebe0' }}>payments@sarego.africa</strong> with your email as reference.</div>
