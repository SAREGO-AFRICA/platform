...
export default function TradeFinanceListingFeePage() {
  ...
  const crd = (s) => ({ background:s?'rgba(184,150,46,0.08)':'rgba(255,255,255,0.03)', border:'1px solid '+(s?'#b8962e':'rgba(255,255,255,0.08)'), borderRadius:12, padding:20, cursor:'pointer' });
  const NAV = <div style={{ background:'#0d0f13', borderBottom:'1px solid rgba(184,150,46,0.15)', padding:'12px 24px', display:'flex', alignItems:'center', gap:16 }}><a href="/" style={{ display:'flex', alignItems:'center', gap:8, textDecoration:'none' }}><span style={{ color:'#b8962e', fontSize:18 }}>◈</span><span style={{ color:'#e8e0d0', fontSize:14, fontWeight:600 }}>SAREGO</span></a><span style={{ color:'rgba(232,224,208,0.2)' }}>|</span><a href="/my-listings" style={{ color:'rgba(232,224,208,0.5)', fontSize:13, textDecoration:'none' }}>My Listings</a><a href="/account" style={{ color:'rgba(232,224,208,0.5)', fontSize:13, textDecoration:'none', marginLeft:'auto' }}>My Account</a></div>;
  if (done) return <div style={pg}>{NAV}<div style={{ ...inn, textAlign:'center', paddingTop:60 }}><h2 style={{ fontSize:24, fontWeight:700, marginBottom:12 }}>Listing Boosted</h2><p style={{ color:'rgba(232,224,208,0.6)', marginBottom:32 }}>Priority placement active within 24 hours of payment confirmation.</p><Link to="/my-listings" style={{ background:'#b8962e', color:'#0b0d10', padding:'12px 32px', borderRadius:8, fontWeight:700, textDecoration:'none' }}>View My Listings</Link></div></div>;
  return (
    <div style={pg}>{NAV}<div style={inn}>
      <h1 style={{ fontSize:28, fontWeight:700, margin:'32px 0 8px' }}>Finance Listing Packages</h1>
      <p style={{ fontSize:14, color:'rgba(232,224,208,0.5)', margin:'0 0 24px' }}>Boost visibility for your trade finance request</p>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:24 }}>
        {PKGS.map(p => (<div key={p.id} style={crd(sel===p.id)} onClick={()=>setSel(p.id)}><div style={{ fontSize:14, fontWeight:600, marginBottom:8 }}>{p.label}</div><div style={{ fontSize:24, fontWeight:800, color:p.price===0?'#888':'#b8962e', marginBottom:8 }}>{p.price===0?'Free':'$'+p.price}</div><div style={{ fontSize:12, color:'rgba(232,224,208,0.5)', marginBottom:10 }}>{p.desc}</div>{p.features.map((f,i)=><div key={i} style={{ fontSize:12, color:'rgba(232,224,208,0.7)', padding:'2px 0' }}>✓ {f}</div>)}</div>))}
      </div>
      {pkg.price>0 && (<div style={sec}>
        ...
        <div style={{ fontSize:13, color:'rgba(232,224,208,0.5)', lineHeight:1.6, padding:'12px 0', borderTop:'1px solid rgba(255,255,255,0.06)' }}>Send ${pkg.price} USD to <strong style={{ color:'#e8e0d0' }}>payments@sarego.africa</strong></div>
