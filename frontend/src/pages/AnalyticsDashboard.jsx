...
export default function AnalyticsDashboard() {
  ...
  const s = { page: { minHeight:'100vh', background:'#f5f0e8', color:'#f0ebe0', fontFamily:'Inter Tight, sans-serif' }, inner: { maxWidth:1100, margin:'0 auto', padding:'0 24px 80px' }, card: { background:'rgba(0,0,0,0.04)', border:'1px solid rgba(184,150,46,0.15)', borderRadius:12, padding:20 }, lbl: { fontSize:10, color:'#b8962e', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:6, fontWeight:600 } };
  ...
  if (error && (error.includes('Premium') || error.includes('requires') || error.includes('403'))) return (
    <div style={s.page}>
      <div style={{ ...s.inner, textAlign:'center', paddingTop:80 }}>
        ...
        <p style={{ color:'rgba(26,26,26,0.6)', marginBottom:32, maxWidth:480, margin:'0 auto 32px' }}>Market intelligence requires a Verified Business membership or higher.</p>
        <Link to="/pricing" style={{ background:'#b8962e', color:'#f5f0e8', padding:'12px 32px', borderRadius:8, fontWeight:700, textDecoration:'none', fontSize:15 }}>Upgrade to Access Analytics</Link>
      </div>
    </div>
  );
  ...
  return (
    <div style={s.page}>
      <div style={{ background:'#ece8df', borderBottom:'1px solid rgba(184,150,46,0.15)', padding:'12px 24px', display:'flex', alignItems:'center', gap:16 }}>
        <a href="/" style={{ display:'flex', alignItems:'center', gap:8, textDecoration:'none' }}><span style={{ color:'#b8962e', fontSize:18 }}>◈</span><span style={{ color:'#f0ebe0', fontSize:14, fontWeight:600, letterSpacing:'0.05em' }}>SAREGO</span></a>
        <span style={{ color:'rgba(232,224,208,0.2)' }}>|</span>
        <a href="/dashboard" style={{ color:'rgba(26,26,26,0.55)', fontSize:13, textDecoration:'none' }}>Dashboard</a>
        <a href="/account" style={{ color:'rgba(26,26,26,0.55)', fontSize:13, textDecoration:'none', marginLeft:'auto' }}>My Account</a>
      </div>
      <div style={s.inner}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', margin:'32px 0' }}>
          <div><h1 style={{ fontSize:28, fontWeight:700, margin:'0 0 8px' }}>Market Intelligence</h1><div style={{ fontSize:14, color:'rgba(26,26,26,0.55)' }}>SADC trade analytics</div></div>
