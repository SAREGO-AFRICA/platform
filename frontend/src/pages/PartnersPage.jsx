...
export default function PartnersPage() {
  ...
  const s = {
    ...
    stat:    { background:'rgba(255,255,255,0.03)', border:'1px solid rgba(184,150,46,0.1)', borderRadius:10, padding:'16px 20px', textAlign:'center' },
    lbl:     { fontSize:10, color:'#b8962e', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:6, fontWeight:600 },
    tab:     (a) => ({ padding:'8px 20px', borderRadius:20, border:'none', cursor:'pointer', fontSize:13, fontWeight:600, background:a?'#b8962e':'rgba(255,255,255,0.06)', color:a?'#0b0d10':'rgba(232,224,208,0.6)' }),
    ...
    inp:     { background:'#1a1a1a', color:'#e8e0d0', border:'1px solid #333', borderRadius:6, padding:'10px 14px', fontSize:13, flex:1 },
    btn:     { background:'#b8962e', color:'#0b0d10', border:'none', borderRadius:8, padding:'10px 20px', fontWeight:700, fontSize:13, cursor:'pointer' },
  };
  ...
  if (notPartner || !profile) return (
    <div style={s.page}>
      ...
      <div style={{ ...s.inner, textAlign:'center', paddingTop:80 }}>
        ...
        <p style={{ fontSize:16, color:'rgba(232,224,208,0.6)', maxWidth:560, margin:'0 auto 40px', lineHeight:1.7 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:16, maxWidth:700, margin:'0 auto 48px', textAlign:'left' }}>
          {[
            ...
            <div key={item.label} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(184,150,46,0.15)', borderRadius:10, padding:16 }}>
        ...
        <div style={{ marginTop:16, fontSize:13, color:'rgba(232,224,208,0.4)' }}>Free to join. Commissions paid on verified revenue events only.</div>
      </div>
    </div>
  );
      ...
      <div style={s.inner}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:32 }}>
          <div>
            ...
            <div style={{ fontSize:14, color:'rgba(232,224,208,0.5)' }}>Referral Code: <strong style={{ color:'#b8962e' }}>{profile.referral_code}</strong></div>
          </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:28 }}>
          <div style={s.stat}><div style={{ fontSize:28, fontWeight:700, color:'#b8962e' }}>{profile.lifetime_referrals}</div><div style={{ fontSize:12, color:'rgba(232,224,208,0.5)', marginTop:4 }}>Total Referrals</div></div>
          <div style={s.stat}><div style={{ fontSize:28, fontWeight:700, color:'#f59e0b' }}>{fmtUSD(stats?.commissions?.pending)}</div><div style={{ fontSize:12, color:'rgba(232,224,208,0.5)', marginTop:4 }}>Pending</div></div>
          <div style={s.stat}><div style={{ fontSize:28, fontWeight:700, color:'#22c55e' }}>{fmtUSD(stats?.commissions?.approved)}</div><div style={{ fontSize:12, color:'rgba(232,224,208,0.5)', marginTop:4 }}>Approved</div></div>
          <div style={s.stat}><div style={{ fontSize:28, fontWeight:700, color:'#e8e0d0' }}>{fmtUSD(stats?.commissions?.paid)}</div><div style={{ fontSize:12, color:'rgba(232,224,208,0.5)', marginTop:4 }}>Paid Out</div></div>
        </div>
        ...
        <div style={{ ...s.card, marginBottom:24 }}>
          ...
          <div style={{ display:'flex', gap:0, borderRadius:6, overflow:'hidden', height:8 }}>
            {[
              ...
              return <div key={t.key} style={{ flex:1, background: isPast||isActive ? TIER_META[t.key].color : 'rgba(255,255,255,0.06)', opacity: isActive ? 1 : isPast ? 0.5 : 0.2 }} />;
            })}
          </div>
          <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, color:'rgba(232,224,208,0.3)', marginTop:6 }}>
        ...
        {tab==='overview' && (
          <div style={s.card}>
            ...
            {referrals.length===0 && <div style={{ color:'rgba(232,224,208,0.4)', fontSize:13, padding:'16px 0' }}>No referrals yet. Share your referral link to get started.</div>}
            {referrals.slice(0,5).map(r => (
              <div key={r.id} style={s.row}>
                <div>
                  <div style={{ fontSize:14 }}>{r.referred_email||r.referred_name||'Anonymous'}</div>
                  <div style={{ fontSize:12, color:'rgba(232,224,208,0.4)', marginTop:2 }}>{fmtDate(r.created_at)}</div>
                </div>
        ...
        {tab==='referrals' && (
          <div style={s.card}>
            ...
            {referrals.length===0 && <div style={{ color:'rgba(232,224,208,0.4)', fontSize:13, padding:'16px 0' }}>No referrals yet.</div>}
            {referrals.map(r => (
              <div key={r.id} style={s.row}>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:14 }}>{r.referred_email||r.referred_name||'Anonymous'}</div>
                  <div style={{ fontSize:12, color:'rgba(232,224,208,0.4)', marginTop:2 }}>Joined {fmtDate(r.created_at)} · {r.membership_tier||'free'}</div>
                </div>
                <span style={s.badge(STATUS_COLOR[r.status]||'#888')}>{r.status}</span>
              </div>
            ))}
        ...
        {tab==='earnings' && (
          <div style={s.card}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              ...
              <select value={commFilter} onChange={e=>setCommFilter(e.target.value)} style={{ background:'#1a1a1a', color:'#e8e0d0', border:'1px solid #333', borderRadius:6, padding:'6px 10px', fontSize:12 }}>
            ...
            {filteredComm.length===0 && <div style={{ color:'rgba(232,224,208,0.4)', fontSize:13 }}>No commissions yet.</div>}
            {filteredComm.map(c => (
              <div key={c.id} style={s.row}>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:14, textTransform:'capitalize' }}>{c.revenue_type.replace(/_/g,' ')}</div>
                  <div style={{ fontSize:12, color:'rgba(232,224,208,0.4)', marginTop:2 }}>{fmtDate(c.created_at)} · {c.commission_percentage}% of {fmtUSD(c.gross_amount)}</div>
                </div>
        ...
        {tab==='tools' && (
          <div style={s.card}>
            ...
            <div style={{ marginBottom:20 }}>
              <div style={{ fontSize:13, color:'rgba(232,224,208,0.6)', marginBottom:10 }}>Your Referral Link</div>
            ...
            <div>
              <div style={{ fontSize:13, color:'rgba(232,224,208,0.6)', marginBottom:10 }}>Your Referral Code</div>
            ...
            <div style={{ marginTop:24, padding:16, background:'rgba(184,150,46,0.06)', borderRadius:8, border:'1px solid rgba(184,150,46,0.15)' }}>
              <div style={{ fontSize:12, color:'rgba(232,224,208,0.6)', lineHeight:1.7 }}>
