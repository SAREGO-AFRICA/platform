...
function ListingCard({ item, type, linkBase }) {
  return (
    <Link to={'/opportunities/'+linkBase+'/'+item.id} style={{ textDecoration:'none' }}>
      <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:10, padding:'14px 16px', marginBottom:10, transition:'border-color 0.2s' }}
        onMouseEnter={e => e.currentTarget.style.borderColor='rgba(184,150,46,0.3)'}
        onMouseLeave={e => e.currentTarget.style.borderColor='rgba(255,255,255,0.07)'}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12 }}>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:14, fontWeight:500, color:'#e8e0d0', marginBottom:4 }}>{item.title}</div>
            <div style={{ fontSize:12, color:'rgba(232,224,208,0.5)' }}>
...
export default function CorridorDetailPage() {
  ...
  const s = {
    ...
    stat: { background:'rgba(255,255,255,0.03)', border:'1px solid rgba(184,150,46,0.15)', borderRadius:10, padding:'16px 20px', textAlign:'center' },
  };
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.inner}>
        {/* Breadcrumb */}
        <div style={{ fontSize:12, color:'rgba(232,224,208,0.4)', marginBottom:24 }}>
        ...
        <div style={{ marginBottom:32 }}>
          ...
          <h1 style={{ fontSize:'clamp(24px,3vw,38px)', fontWeight:500, margin:'0 0 8px', letterSpacing:'-0.01em' }}>
            {cn(origin)} <span style={{ color:'rgba(232,224,208,0.3)' }}>→</span> {cn(destination)}
          </h1>
          <div style={{ fontSize:15, color:'rgba(232,224,208,0.5)' }}>{origin} → {destination} economic corridor</div>
        </div>
        ...
        {data && (
          <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16, marginBottom:32 }}>
              <div style={s.stat}><div style={{ fontSize:32, fontWeight:700, color:'#b8962e' }}>{data.summary?.total_flows||0}</div><div style={{ fontSize:12, color:'rgba(232,224,208,0.5)', marginTop:4 }}>Active Flows</div></div>
              <div style={s.stat}><div style={{ fontSize:32, fontWeight:700, color:'#e8e0d0' }}>{fmtUSD(data.summary?.total_value)}</div><div style={{ fontSize:12, color:'rgba(232,224,208,0.5)', marginTop:4 }}>Total Value</div></div>
              <div style={s.stat}><div style={{ fontSize:32, fontWeight:700, color:'#22c55e' }}>{(data.logistics?.length||0)+(data.trade_finance?.length||0)+(data.commodity?.length||0)}</div><div style={{ fontSize:12, color:'rgba(232,224,208,0.5)', marginTop:4 }}>Listed Opportunities</div></div>
            </div>
            ...
            {data.logistics?.length===0 && data.trade_finance?.length===0 && data.commodity?.length===0 && (
              <div style={{ textAlign:'center', padding:48, color:'rgba(232,224,208,0.4)' }}>No active listings on this corridor.</div>
            )}
          </>
        )}
      </div>
