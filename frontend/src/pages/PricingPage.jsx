...
export default function PricingPage() {
  ...
  const s = {
    ...
    title: { fontSize: 42, fontWeight: 700, color: '#f0ebe0', margin: '0 0 12px' },
    sub:   { fontSize: 18, color: 'rgba(26,26,26,0.65)', margin: '0 0 32px' },
    toggle: { display: 'inline-flex', background: 'rgba(0,0,0,0.07)', borderRadius: 24, padding: 4, gap: 2 },
    toggleBtn: (a) => ({ padding: '8px 20px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, background: a ? '#b8962e' : 'transparent', color: a ? '#f5f0e8' : 'rgba(26,26,26,0.65)' }),
    ...
    card:  (color, current) => ({ background: current ? 'rgba(184,150,46,0.06)' : 'rgba(255,255,255,0.03)', border: `1px solid ${current ? color : 'rgba(0,0,0,0.1)'}`, borderRadius: 16, padding: 28, display: 'flex', flexDirection: 'column', position: 'relative' }),
    badge: (color) => ({ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: color, color: '#f5f0e8', fontSize: 10, fontWeight: 800, padding: '3px 12px', borderRadius: 10, letterSpacing: '0.1em', whiteSpace: 'nowrap' }),
    tierName: { fontSize: 20, fontWeight: 700, color: '#f0ebe0', margin: '0 0 4px' },
    tagline: { fontSize: 13, color: 'rgba(26,26,26,0.6)', margin: '0 0 20px' },
    price:   { fontSize: 40, fontWeight: 800, color: '#f0ebe0', margin: '0 0 4px' },
    ...
    btn:     (color, disabled) => ({ marginTop: 'auto', paddingTop: 24, background: disabled ? 'rgba(0,0,0,0.07)' : color, color: disabled ? 'rgba(26,26,26,0.45)' : '#f5f0e8', border: 'none', borderRadius: 8, padding: '12px 20px', fontWeight: 700, fontSize: 14, cursor: disabled ? 'default' : 'pointer', width: '100%' }),
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ textAlign: 'center', padding: '0 24px 64px', color: 'rgba(26,26,26,0.45)', fontSize: 13 }}>
