...
export default function CorridorsIndexPage() {
  ...
  const tabStyle = (active) => ({
    ...
    background: active ? '#b8962e' : 'rgba(255,255,255,0.06)',
    color: active ? '#0b0d10' : 'rgba(232,224,208,0.6)',
  });
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0b0d10', color:'#e8e0d0', fontFamily:'Inter Tight, sans-serif' }}>
      ...
      <div style={{ maxWidth:1000, margin:'0 auto', padding:'48px 24px' }}>
        ...
        {/* Stats row */}
        {!loading && (
          <div style={{ display:'flex', gap:24, marginBottom:32, fontSize:13, color:'rgba(232,224,208,0.5)' }}>
            ...
            return (
              <Link key={c.origin+'-'+c.destination} to={'/corridors/'+c.origin+'-'+c.destination} style={{ textDecoration:'none' }}>
                <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(184,150,46,0.15)', borderRadius:12, padding:20, transition:'border-color 0.2s', position:'relative' }}
                  ...
                  <div style={{ fontSize:13, color:'rgba(232,224,208,0.6)', marginBottom:12 }}>{cn(c.origin)}{isDomestic ? ' (domestic)' : ' → '+cn(c.destination)}</div>
        ...
        {!loading && sorted.length === 0 && (
          <div style={{ textAlign:'center', padding:64, color:'rgba(232,224,208,0.4)' }}>No corridors found for this filter.</div>
        )}
      </div>
    </div>
  );
